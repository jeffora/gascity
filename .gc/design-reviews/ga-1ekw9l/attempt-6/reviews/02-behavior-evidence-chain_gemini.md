# Oleg Marchetti — DeepSeek V4 Flash (Behavior Evidence Chain Reviewer, Attempt 6, Independent Review)
...
Reviewed against the updated design document in Attempt 6 (`plans/core-gastown-pack-migration/implementation-plan.md` / `plans/core-gastown-pack-migration/requirements.md`) and grounded in the live codebase, prior iterations, and other reviewer outputs.

---

## Executive Summary

As Oleg Marchetti, the **Behavior Evidence Chain Reviewer** (role: **cross-repo-evidence-chain-auditor**), I have conducted an independent, evidence-backed review of the Attempt 6 (Iteration 6) design. 

While the design has matured significantly—especially in the clear separation of rollout slices and the documentation of prerequisites—my verdict remains **Verdict: block**.

The design still exhibits structural gaps and loopholes in the behavior-evidence contract. In particular, it maintains a file-level "asset" granularity instead of discrete "trigger" granularity, allows the "source assertion" loophole to bypass actual execution-level witness checks, suffers from a circular bootstrap dependency, and lacks a reconciliation mechanism between the Gas City and `gascity-packs` manifests. These flaws must be resolved programmatically before the source-deletion gate can be approved.

---

## Top Strengths & Design Evolution

- **Pin-Before-Delete Sequencing**: The sequencing in "External Public Gastown Prerequisite" (Lines 104–144) is highly rigorous, preventing Gas City source deletions or synthetic alias retirements until public Gastown work exists at immutable commits.
- **Differentiated Pin Meanings**: The separation of `PublicGastownPackVersion` into a `compatibility` pin (proving coexistence) and an `activation` pin (proving no-Maintenance loading) in the same candidate branch (Lines 139–144) remains an elegant solution to staging.
- **Git Historical Baseline integration**: The generator's reliance on a Git historical baseline (Lines 183–185) prevents developers from quietly deleting legacy files without creating corresponding manifest rows.
- **Clear packcompat Isolation**: `test/packcompat` correctly consumes the pinned public checkout rather than copied workspace assets (Lines 187–191), enforcing a clean external behavior boundary.
...
- **The Assumption**: The plan asserts that CI fails if a moved, split, generalized, deleted, or helper-dependent "asset" lacks a row (Lines 181–183).
- **The Reality**: An "asset" is a file-level abstraction. However, a single TOML formula/order, shell script, or prompt template routinely contains multiple independent triggers, routes, detectors, or prompts. For instance, the two moving packs carry **217 trigger-bearing lines** across ~54 assets.
- **The Blocker**: If a single file contains multiple triggers, registering one row for that "asset" satisfies the CI completeness gate while leaving the majority of its constituent triggers completely unrepresented, unverified, and unprotected.
- **Required Change**: State explicitly that the manifest must carry **one row per discrete trigger** (each requester, detector, notification, mail, nudge, prompt fragment, order/escalation gate, and script branch), and that the CI completeness gate must fail if a *trigger*—not merely an asset—lacks a row. Add an automated trigger-count reconciliation (comparing extracted-trigger counts against row counts per asset) so that any asset with $N$ triggers and $< N$ rows fails closed.

### 2. [Blocker] Witness-Strength Loophole for Execution-Level Triggers
- **The Assumption**: The row schema allows "source assertion" as an acceptable old witness type (Line 165) and "static scanner" as an acceptable witness-kind (Line 167).
...
- **The Blocker**: High-risk, complex behaviors (such as detectors, notification deliveries, or script-run outcomes) can pass the gate with zero automated execution verification on either side, simply by citing code existence. This undercuts the execution-level witness guarantee.
- **Required Change**: Introduce a strict **Witness-Strength Matrix** specifying that for trigger-kinds (`detector`, `requester`, `notification`, `mail`, `nudge`, and `order/escalation gate`), the new witness-kind must be execution-level (process/integration test, a packcompat assertion that actually fires the trigger, or manual proof transcript); `static scanner` and `source assertion` are disallowed for these kinds. Explicitly define that `packcompat`'s per-row check must execute and fire the trigger for these kinds, and can only use presence verification for genuinely non-executable prose rows (which then require a named human equivalence attestation).

### 3. [Blocker] Cross-Repo Bootstrap Lock / Circular Deadlock
- **The Assumption**: The plan assumes `gascity-packs/gastown` will produce `behavior-preservation.yaml` and `public-gastown-pins.yaml` (Lines 115–118) as a prerequisite for Gas City's slices, while Gas City's generator will walk old sources to ensure manifest completeness (Lines 178–183).
- **The Reality**: This creates a tight circular dependency. The public pack repository `gascity-packs` has no access to the Gas City source tree in its isolated CI, meaning it cannot programmatically generate equivalence with old sources. Conversely, Gas City's generator requires the immutable public Gastown commit and the new paths (Lines 173–174) to generate its own manifest rows. Neither repository can finalize its manifest because each requires the other's immutable state as an input.
...
- **Required Change**: Define a concrete **bootstrap/staging workflow** (e.g., using a local multi-workspace development mode, a mutable draft-pin branch, or a joint generator tool) to break this deadlock before enforcing immutable SHA checks.

### 4. [Blocker] Two Independent Manifests Lacking Reconciliation
- **The Assumption**: The plan builds a Gas City-side manifest (`internal/packevidence`, `cmd/gc/pack_evidence.go`, `testdata/packevidence/`; Lines 148–154) *and* a gascity-packs-side `gastown/behavior-preservation.yaml` (Lines 115–118, 130–131).
...
- **The Assumption**: The generator "compares against a Git historical baseline so current-workspace-only scans cannot miss deleted or moved legacy behavior" (Lines 183–185).
...
- **The Assumption**: The plan explicitly lists only `examples/gastown/gastown_test.go` and `examples/gastown/maintenance_scripts_test.go` (Lines 113–114) for the cross-repo mapping obligation.
