...
Workflow-root query authority is split deliberately:

| Owner | Authority |
|---|---|
| `internal/formula` | Defines normalized requirement semantics, graph-workflow facts, and exact metadata key/value meanings |
| `internal/sourceworkflow` or successor shared predicate package | Builds bead-store query criteria and post-fetch predicates from the formula helpers |
