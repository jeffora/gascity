...
combination lacks an explicit row, an explicit `unsupported` reason, or an
explicit `impossible` reason. Complete coverage is required for accepted and
rejected strings, every non-string TOML/JSON value kind, duplicate or invalid
TOML shapes, legacy `contract`, legacy `version`, dual declarations, invalid
host capability, construct identity, construct location, contribution path,
caller path, inherited inline children, expansion/aspect/transitive-import
contributions, normalized output, JSON pointer/TOML source attribution, and
diagnostic order/count. A reviewer must be able to add a new requirement axis
or v2-only construct and see exactly which matrix rows, diagnostic fixtures,
docs rows, and tests must change.
...
`molecule.Cook`, `molecule.Attach`, `GraphWorkflow`, and workflow-root metadata
filters. Each row names the owner package, current helper, durable-write
...
type AcceptedCompileArtifact struct {
    ...
    CreatedBy         string
    CreatedAt         time.Time
    seal              acceptedCompileSeal
}
```

The unexported `seal` field is intentional. Code outside `internal/formula`
cannot construct this value directly, and tests in other packages must obtain
one through test-only helpers that call the same acceptance checks. The only
exported readers on the artifact return copied values; no caller can mutate
requirements, diagnostics, provenance, or identity after acceptance.
...
`ProjectAcceptedFormula`. `CreateConvergenceBead`, retry root creation,
missing-child markers, speculative wisps, and the first `PourWisp` call all run
...
when needed, then durable root, metadata, wisp, retry, iteration, or child
writes.
