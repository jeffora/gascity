...
The compiler also emits a versioned projection snapshot that is copied into
accepted artifacts. This is the only data durable convergence, fanout, retry,
scope-check, workflow-finalize, and missing-child repair may use when they need
formula facts after the original source may have moved or disappeared:
...
`CompileWithResult` populates this snapshot from the fully decoded, inherited,
expanded, and source-attributed compiler output. `AcceptCompileResult` copies
the snapshot into `AcceptedCompileArtifact` after verifying fatal diagnostics,
identity fields, host satisfaction, and provenance. `ProjectAcceptedFormula`,
...
- id: cmd-gc-order-dispatch-filter-001
  ...
  replacement_api: internal/sourceworkflow.WorkflowRootQuery
  owner: cmd/gc
  expiry_phase: 4c
  blocking_test: TestOrderDispatchUsesWorkflowRootQuery
```
...
`durable_write_boundary`, `replacement_api`, owner, expiry phase, and blocking
test. CI compares the raw-consumer scan to the manifest after every caller
migration sub-phase. A raw predicate/filter site may be deleted only when the
same change removes its manifest row and adds or updates the replacement test.
...
6. Docs and examples completion: stale live docs, generated CLI/config docs,
   ...
   in migration notes.
7. Pack-floor enforcement: land resolver/import enforcement for `[pack]
   requires_gc`, seed or refresh the compatibility/minimum-floor/external-
   support artifacts, and prove incompatible packs are rejected before formula
...
| legacy-only root, source present, host enabled | pause graph-specific mutation until migration preflight succeeds | `gc formula repair-root-artifact <root-id> --dry-run --json`, then the same command without `--dry-run` stamps only the artifact ref and migration metadata |
