...
production constructor for the legacy boolean, and `HostCapabilitiesForTest`
is available only from `internal/formula/testonly` or `_test.go` files. The
fields are unexported so callers cannot bypass provenance, config-generation,
or value validation with struct literals. Accessors such as
`FormulaCompiler()`, `Source()`, `SourceKind()`, and `ConfigGeneration()` are
read-only projections. Any invalid value is rejected by `CheckRequirements` as
an internal configuration error diagnostic and never panics. Future values such
as `CompilerCapability(3)` are unsupported until the same change adds typed
parser support, diagnostics, metadata, docs, and tests. Satisfaction is
computed for every call from `CompileOptions.HostCapabilities`; it is not
cached on formula identity, process-global config, or pack resolution state.
...
`HostCapabilitiesFromFormulaV2(true, "city.toml:[daemon].formula_v2")`; false
maps through the same constructor with `false`. The boolean exists only as
legacy config vocabulary at the edge; the constructor's typed
`FormulaCompiler()` value is the canonical capability used by
`CheckRequirements`.
...
- Event payloads for compile failures are typed and registered in
  ...
  `requirement_source_value`, `host_source_kind`, `host_source_path`,
  `host_source_key`, `host_source_value`, `normalized_requirement`,
...
PackV2 author-doc link check, and release-report schema validation. The command
...
3. First-party pack-floor declaration: write or update first-party `[pack]
   ...
   the floor yet; it makes the upcoming resolver/import enforcement auditable.
...
7. Pack-floor enforcement: after packman schema `2` or an equivalent
   packman-owned provenance contract lands, enforce `[pack] requires_gc`, seed
...
| 7 pack-floor enforcement | `internal/packman`, resolver/import loading | packman owner | `go test ./internal/packman ./internal/formula` | disable resolver enforcement before formula selection |
...
validated by `ValidateAcceptedArtifact`; `Vars` are checked only against
projected runtime-var requirements; `CurrentHost` is recorded for diagnostics
but cannot reauthorize a changed-identity write. Unknown keys in `Vars` are
reported according to convergence policy; missing required vars fail before
root, iteration, retry, hook, dependency, convoy, or speculative-wisp writes.
