...
TOML/JSON value kind, legacy `contract`, dual declarations, invalid host
capability, each v2-only construct location, inherited inline children,
expansion/aspect/transitive-import contributions, normalized output, source
attribution, and diagnostic order/count. A reviewer must be able to add a new
requirement axis or v2-only construct and see exactly which matrix rows,
...
| valid TOML with unsupported byte-exact string such as `">= 2"` or `">=3"` | structured `formula.compiler_requirement_unsupported` |
...
The v0 grammar is byte-exact: only omitted, `">=1"`, and `">=2"` are accepted.
No trimming, semver parsing, numeric coercion, or optimistic future-range
parsing is allowed in v0.
...
| `unsupported-future-requirement` | current `main` | deterministic `formula.compiler_requirement_unsupported` |
...
- `OnceKey` shape is
  ...
  the formula source, config generation, or subject id changes.
...
The projection API is explicit:
...
func (r *CompileResult) ConvergenceMetadata() (ConvergenceMetadata, []Diagnostic)
```

`ConvergenceMetadata()` may report convergence-domain diagnostics such as a
missing evaluate prompt, but it may not reinterpret raw formula requirement
fields or host capability. `CreateConvergenceBead`, retry root creation,
missing-child markers, speculative wisps, and the first `PourWisp` call all
run after canonical compile, requirement satisfaction, projection, and
convergence-domain validation complete.
