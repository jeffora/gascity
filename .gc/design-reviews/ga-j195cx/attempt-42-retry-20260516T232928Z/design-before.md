...
PackV2 lockfile semantics are owned by `internal/packv2/lockfile.go` and the
schema at `docs/schema/pack-lockfile.v1.json`. The lockfile is the only source
of `LockedRevision` for remote refs and transitive imports:
...
requested_ref = "main"
locked_revision = "sha256-or-git-sha"
content_hash = "sha256:..."
requires_gc = ">=0.x"
...
source, requested ref, locked revision, content hash, `requires_gc`, and every
formula requirement delta. Branch or tag refs are mutable unless a lock entry
pins their resolved revision. Registry sources follow the same rule: a semver
constraint is not reproducible until the lockfile records the resolved immutable
