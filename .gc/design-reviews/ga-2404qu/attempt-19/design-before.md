...
updated_at: 2026-06-09T08:40:42Z
---
...
- Stale `.gc/system/packs/maintenance` and `.gc/system/packs/gastown`
  directories are isolated from active discovery, diagnosed, and preserved
  byte-for-byte, not deleted.
- Core assets remain role-neutral outside explicitly allowed Core maintenance
  configuration.
- Public Gastown activation-pin adoption and Maintenance removal are one
  candidate change. A no-Maintenance gate may run only on a tree where the
  normal production loader already excludes Maintenance; passing that gate is a
  pre-merge condition, not a prerequisite for editing `requiredBuiltinPackNames`.
  <!-- REVIEW: added per attempt13-slice5-gate-ordering -->
- Public Gastown reachability may be proven by the network source or by a
  validated ordinary remote-cache entry for the exact source, commit, and
  subpath. Embedded bytes and retired synthetic aliases never satisfy a public
  pin.
...
Required-pack repair runs before fatal validation publishes an effective pack
tree. Missing or corrupt expected files are regenerated from the embedded
manifest. Unexpected generated files are pruned; unexpected operator-edited or
unclassifiable files are moved to a recovery location outside active discovery
and named in the diagnostic. If repair cannot produce an exact manifest-backed
tree, loading fails closed before any formula, order, script, prompt, hook, or
overlay is read.
<!-- REVIEW: added per attempt13-required-pack-tamper-recovery -->
...
public Gastown source reachability or validated ordinary remote-cache presence,
immutable pin installability, lock generation, Core materialization and content
validation, parseable lockfiles, editable manifests, and generated-source
provenance for any import it plans to remove or rewrite.

The mutation coordinator acquires its crash-released advisory city lock before
file/digest preflight. If a report-only legacy phase reads before the lock for
operator messaging, the coordinator must repeat digest and provenance
validation after the lock and before staging. No public-pin, import-rewrite, or
runtime-state fix slice may expose legacy direct mutations; the coordinator
lands first, or legacy automatic fixes are frozen.
<!-- REVIEW: added per attempt13-doctor-mutation-order -->
...
- offline behavior decision: network source or validated ordinary remote-cache
  hit for the exact source, commit, and subpath; legacy synthetic public
  Gastown cache may only be copied through an explicit promotion helper into
  that ordinary cache key, never selected directly
...
The Core extraction slice deletes the `//go:embed packs/**` directive and the
`embeddedBootstrapPacks` variable rather than leaving an empty embed pattern.
Bootstrap tests use `_test.go`-only inline `fstest.MapFS` fixtures with
synthetic asset paths such as `packs/test-core`; a fixture `Entry.Name` may be
...
on production embeds of `packs/**`, `AssetDir: "packs/core"` in bootstrap
fixtures, or hook overlays loading from the old bootstrap Core path.

The hidden-dependency inventory is explicit, not discovered ad hoc during source
deletion. It includes `cmd/gc/prompt_test.go`, hook overlay tests, packlint
fixtures, config bundled-import tests, generated docs, and any path-string guard
that mentions `internal/bootstrap/packs/core`, `testdata/packs/core`, or
`AssetDir: "packs/core"`.
<!-- REVIEW: added per attempt13-bootstrap-hidden-dependencies -->
...
Provider pack continuity is a gate for `bd` and `dolt`: provenance, install
locks, include behavior, formula/order resolution, and materialization must
remain correct when Core is repaired and Maintenance is absent. Byte continuity
holds for every provider asset not intentionally rewritten for role-cleaning or
target-binding conversion. Any rewritten provider asset must have a behavior
manifest row, old/new witness tests, and a provenance record naming the exact
semantic delta. Required system-pack integrity must validate the effective file
set and prune or quarantine unexpected effective files before behavior
discovery.

Provider-pack byte-continuity exceptions are narrow and table-driven:
<!-- REVIEW: added per attempt13-provider-pack-role-cleaning -->

| Pack | Allowed rewrite | Required evidence |
| --- | --- | --- |
| `bd` | No behavior rewrite unless the generated role-surface inventory finds an active role-bearing command, doctor, script, or template surface. Historical issue text and upstream comments are docs-only rows unless they affect operator output. | Role-surface row or explicit no-op row, provider matrix for `gc-beads-bd`, materialized digest proof for untouched files. |
| `dolt` | Rewrite active formula/order/script targets such as `mol-dog-backup`, `mol-dog-doctor`, `mol-dog-stale-db`, `mol-dog-phantom-db`, `mol-dog-compactor`, and related mail/nudge recipients from hardcoded `dog`, `mayor/`, or `deacon/` targets to `core.maintenance_worker` bindings and configured recipients. | Old/new behavior witnesses for backup, doctor, stale-db, phantom-db, compaction, health JSON consumers, order pool resolution, and configured-recipient empty-value handling. |
...
- Generated or known example Gastown imports are rewritten to
  ...
  `PublicGastownPackVersion` only after the commit is reachable through the
  network source or a validated ordinary remote-cache entry, installable, and
  lockable.
...
| --- | --- | --- |
| `.gc/runtime/packs/maintenance/jsonl-export-state.json` | `.gc/runtime/packs/core/jsonl-export-state.json` | `gc doctor --fix` and the first Core `jsonl-export.sh` run copy when Core destination is absent, then record migration completion. If both exist before completion is recorded and differ, report manual conflict; after completion, Core is live and legacy is retained rollback state. |
...
| `.gc/runtime/packs/maintenance/jsonl-archive/` | `.gc/runtime/packs/core/jsonl-archive/` | Copy only when Core destination is absent; preserve git remotes and refs; if both exist before completion is recorded, diagnose and do not merge. |
...
| `.gc/runtime/packs/maintenance/spawn-storm-counts.json` | `.gc/runtime/packs/core/spawn-storm-counts.json` | Copy when Core destination is absent and record completion; if both exist afterward, Core wins and legacy is reported as ignored rollback state. |
...
The migration-completed marker is stored with the Core runtime state and records
legacy source path, Core destination path, source digest, initial Core digest,
binary/build id, and migrated-at timestamp for each copied state file or
archive. Once the marker exists, new binaries treat later legacy Maintenance
divergence as retained rollback state, not as a conflict. Conflict diagnostics
apply only before the marker exists or when the marker's recorded source digest
does not match the source that was copied.
<!-- REVIEW: added per attempt13-runtime-state-marker -->

Runtime-state migration is non-destructive. Rollback guidance must say that old
binaries may continue reading retained legacy Maintenance state; doctor-mutated
city manifests must remain readable by old binaries unless a release explicitly
names a one-way boundary and recovery path.
...
- no-Maintenance production-loader compatibility: runs on the same candidate
  tree that removes Maintenance from `requiredBuiltinPackNames`, after
  Core-owned Maintenance behavior has moved to Core in that tree
...
loader bypass may be used to satisfy that gate, and the candidate cannot merge
...
3. Prove public Gastown reachability or an exact validated ordinary remote-cache
   hit, exact commit installability, ordinary remote-cache identity, and lock
   generation.
4. Stage Core materialization in a temp directory and validate strict manifest
   integrity before touching user files.
5. Stage runtime-state copies with migration-completion records, reversible
   names, and conflict checks; never delete legacy Maintenance state.
...
If any preflight, staged operation, compare-before-rename, runtime-state copy,
...
| `requiredBuiltinPackNames` | Remove Maintenance in the activation candidate tree after Core-owned Maintenance behavior has moved in that tree; merge only after packcompat no-Maintenance mode is green on that same tree. | Normal production loader test, not test-only includes. |
...
self-heal without network and that untouched provider assets are byte-identical
aside from intentional generated markers. Intentional role-cleaning rewrites use
the provider exception table above. No slice may claim "no Maintenance" until
the same production path operators run excludes Maintenance from required
includes.
...
TOML through a CST/span-preserving editor, plans runtime-state copies, validates
...
| `requiredBuiltinPackNames` | Removes Maintenance in the activation candidate tree after Core-owned behavior moved in that tree; merge waits for no-Maintenance production-loader packcompat on the same tree. | Maintenance folding. |
...
- behavior rows that would duplicate still-bundled Maintenance definitions are
  omitted from the active compatibility asset set or recorded as approved
  semantic deltas until the no-Maintenance loader slice consumes the activation
  pin. There is no inactive compatibility-asset loader mechanism.

The activation pin is consumed by the same candidate Gas City tree that moves
Core-owned Maintenance behavior, removes Maintenance from required system packs,
and runs no-Maintenance packcompat through the normal production loader. That
candidate cannot merge until the gate is green. If the public pack cannot split
compatibility and activation safely, the Gas City slice must use a paired
cross-repo activation/removal boundary and cannot land a public pin while
...
2. Refuse automatic multi-file fixes when a controller for the same city is
   running, then take a crash-released OS advisory lock on the city directory
   file descriptor before preflight reads are trusted.
3. Prove generated-source provenance for every local import removal/rewrite.
4. Prove public Gastown reachability or an exact validated ordinary remote-cache
   hit, exact commit installability, ordinary remote cache identity,
   registry/direct-source equality, and lock generation.
...
6. Plan runtime-state copies with conflict checks, migration-completion records,
   and reversible temporary names; never delete legacy Maintenance state.
...
- warmup and Core mail/nudge targets: resolved from formula, order, pack, city,
  or session metadata. Missing optional recipients mean no warmup/nudge, not a
  hardcoded fallback; required recipient fields fail preflight if empty or `/`.
  A configured recipient whose mail/nudge delivery fails is a non-zero script or
  formula failure, not a swallowed warning.
...
`AssetDir: "packs/core"` in any bootstrap fixture, nil `bootstrapAssets`,
empty-embed leakage into production, and hook overlays resolving from the old
bootstrap Core path. Bootstrap tests that need a Core-like identity use
synthetic asset paths such as `packs/test-core` and do not copy or hash the
shipping Core tree.
...
- compatibility pin: the first public Gastown pin consumed while Maintenance is
  still active
- activation pin: the public Gastown pin consumed by the same candidate tree
  that removes Maintenance, with no-Maintenance production loading required
  green before merge
...
| Core loading/doctor | whole-production loader scanner, API/controller/no-refresh/session/dispatch/import/check/install bypass tests, doctor CST preservation, staged overlay validation, controller-active refusal, failure-injection suite, `make test-cmd-gc-process-parallel`, and `make test-integration-shards-parallel` shards that cover controller reload and config repair. |
| Maintenance folding | no-Maintenance production-loader packcompat, zero duplicate active definitions, runtime-state migration/completion-record/conflict tests, stale directory isolation/preservation, provider `bd`/`dolt` role-cleaning provenance and byte-continuity for untouched assets, `make test-cmd-gc-process-parallel`, and `make test-integration-shards-parallel`. |
| registry/cache cleanup | public synthetic alias retirement, source-plus-version repo cache key, old synthetic cache rejection, public pin remote-cache digest check, bundled Core/`bd`/`dolt` validation, hermetic packcompat fixture/cache mode, `make test-cmd-gc-process-parallel`, and `make test-integration-shards-parallel`. |
...
boundary and manual recovery steps. Runtime-state migration copies legacy
Maintenance state into Core and records completion; old binaries may continue
reading the retained legacy state, while new binaries ignore expected
post-migration digest divergence between live Core state and retained legacy
rollback state.
...
`internal/systempacks` repair is prune-or-quarantine before final validation:
missing/corrupt expected files are restored from embedded source, unexpected
generated files are pruned, and unexpected operator-edited or unclassifiable
files are quarantined outside active discovery with an operator diagnostic. The
fatal integrity gate validates the repaired effective tree; if repair cannot
produce an exact manifest match, the command fails closed.
<!-- REVIEW: added per attempt13-required-pack-quarantine -->
...
Gastown, or delete source only in an activation candidate tree that also updates
`PublicGastownPackVersion` to the activation commit. The no-Maintenance
packcompat gate runs on that candidate before merge. If the public pack cannot
support two safe commits, the compatibility and activation slices collapse into
one paired cross-repo activation/removal boundary with release notes naming the
one-way point.
...
- validates public Gastown reachability or an exact validated ordinary
  remote-cache hit, exact pin installability, lock parseability, Core file-set
  integrity, generated-source provenance, editable TOML spans, and runtime-state
  conflicts before staging any write;
...
1. phase-one report: classify intended changes and refusal reasons without
   mutating live files;
2. concurrency: refuse if a live controller owns the city; otherwise take a
   crash-released OS advisory lock on the city directory file descriptor, not a
   PID, status, or lock file;
3. preflight: validate public Gastown reachability or an exact validated
   ordinary remote-cache hit, exact pin installability, ordinary remote-cache
   identity, Core strict file-set integrity, editable TOML spans, parseable
   locks, generated-source provenance, and runtime-state conflicts;
...
in place and is classified as ignored legacy state. The publish step records a
migration-completed marker containing source path, destination path, source
digest, initial destination digest, and migrated-at binary/build id. If both
sides exist before that marker, differing digests are a manual conflict. After
the marker exists, expected divergence between live Core state and retained
legacy Maintenance state is ignored and does not block automatic fixes. Healthy
cities must be byte-identical after `gc doctor --fix`; repeated fixes must
converge to the same phase-two report.
...
pin, lock refresh, direct source install, stale-cache repair, or fresh offline
init. Existing offline upgrades may run a one-time cache-promotion helper that
copies a validated legacy synthetic public Gastown cache into the ordinary
remote cache key for the exact pinned source, commit, and subpath before
synthetic-source resolution is retired. `SyntheticContentHash` remains only for
bundled Core, `bd`, and `dolt`.
...
new binary/old pack, downgrade after doctor fixes, and hermetic packcompat
execution from a local fixture or pre-populated ordinary remote cache.
...
### Attempt 14 Review Resolution Contracts
<!-- REVIEW: added per attempt14-global-blockers -->

Attempt 14 keeps the design review verdict at `iterate`. The current synthesis
is a block, so the contracts below are binding for implementation beads and
supersede any older wording that can be read as allowing path-only Core proof,
plain doctor mutation, retired-pack fallback, synthetic public Gastown
activation, role-cleaning without provider provenance, broad slice gates, or
docs-only behavior preservation.

#### Resolver-Produced Required-Pack Provenance
<!-- REVIEW: added per attempt14-required-core-participation -->

Required host-pack participation is produced by the resolver, not inferred by a
caller after loading. `internal/systempacks` passes validated required packs to
config resolution as typed base layers, and the resolver returns one
`RequiredSystemPackParticipation` record per active host pack. Each record
contains:

- stable required pack id: `core`, `bd`, or `dolt`
- required-pack class: Core host pack or provider host pack
- embedded source id, binary build id, and source content digest
- expected full file-set digest, `pack.toml` digest, and effective-file digest
- materialized path and repair state before resolution
- resolver layer id, import edge id, and import edge kind
- whether the edge came from an implicit required-host include or an explicit
  operator import diagnosed before resolution
- collision and retired-source classifier result
- load mode: runtime, no-refresh runtime, partial doctor, partial edit, or
  test-only fixture
- validation diagnostic id and fatal/nonfatal classification

Layer ids and import edge ids are deterministic for a given ordered input set.
They are derived from the normalized source identity, content digest, layer
ordinal, and import parent id rather than filesystem paths. A required pack
that materializes successfully but has no resolver-produced participation
record for the same digest fails the load.

The failure semantics are table-driven:

| Failure class | Runtime load | No-refresh runtime load | Plain `gc doctor` | `gc doctor --fix` |
| --- | --- | --- | --- | --- |
| missing expected Core or provider file | repair before publish, then fail closed if still invalid | validate existing files only; fail closed, no refresh | report missing, no mutation | stage repair through coordinator |
| corrupt or stale expected file | repair before publish, then fail closed if still invalid | fail closed, no refresh | report digest mismatch, no mutation | stage repair through coordinator |
| partial materialization | repair before publish, then fail closed if still invalid | fail closed, no refresh | report partial tree, no mutation | stage repair through coordinator |
| unexpected generated file | prune before validation | fail closed unless already outside active discovery | report generated extra file, no mutation | stage prune through coordinator |
| unexpected operator-edited or unknown file | quarantine outside active discovery before validation, or fail closed if quarantine cannot be staged | fail closed, no refresh | report manual/quarantine candidate, no mutation | stage quarantine only with preserving publish record |
| same-name `core`, `bd`, or `dolt` collision | fail before behavior discovery | fail before behavior discovery | report collision, no mutation | fix only generated redundant imports; custom collisions remain manual |
| retired Maintenance/Gastown active import | fail before behavior discovery after pre-resolution diagnostic path | fail before behavior discovery | report retired import, no mutation | rewrite/remove only generated/example imports |
| required pack materialized but not resolved | fail post-resolution | fail post-resolution | report missing participation, no mutation | re-run staged validation; failure aborts publish |
| raw lower-level loader on production behavior path | scanner failure | scanner failure | allowed only through partial doctor wrapper | not allowed outside coordinator |

Every production config resolution path must either enforce both fatal gates or
name a partial-read exception in
`plans/core-gastown-pack-migration/loader-inventory.generated.yaml`. Exception
rows include file, function, loader class, consumed fields, reason, owner,
expiry slice, and focused test. Partial-read helpers may parse import tables,
sources, versions, comments, and lock records needed for diagnostics or
preserving edits; they may not evaluate agents, formulas, orders, hooks,
prompts, overlays, patches, scripts, or route metadata.

#### Read-Only Doctor Diagnostic Boundary
<!-- REVIEW: added per attempt14-doctor-read-only-boundary -->

Plain `gc doctor` is read-only for this migration. It may call only one of
these APIs:

- `internal/systempacks.ValidateRequiredFileSetsNoRefresh`
- `internal/systempacks.LoadRuntimeCityNoRefresh`
- `internal/packsource.ClassifyRawImport`
- the pre-resolution import parser for city and rig TOML
- direct required-file-set validation against the embedded manifest

Plain doctor must not call materializing runtime loaders, pack installers,
repair helpers, cache promotion helpers, import rewriters, TOML writers,
lockfile writers, runtime-state migrators, quarantine/prune writers, or public
pin refresh paths. If a diagnostic cannot be computed without mutation, it
reports `diagnostic_unavailable_without_fix` and points to `gc doctor --fix`.

All mutation happens only through `doctor.MutationCoordinator` and only after
`--fix` is present. Legacy direct fixes are frozen before the public-pin slice
exposes new public Gastown diagnostics. The coordinator owns one advisory
city-directory lock protocol, one staged plan, one compare-before-rename
publish boundary, one publish record schema, and one failure-injection suite.

Failure-injection coverage must include:

| Injection point | Required assertion |
| --- | --- |
| controller active before preflight | automatic fix refuses and live files stay byte-identical |
| second live doctor process | automatic fix refuses using live process discovery, not persistent status files |
| public pin offline and not in ordinary cache | report actionable offline failure; no embedded fallback |
| target TOML changed after preflight | compare-before-rename aborts with changed target diagnostic |
| partial staged Core repair | live required-pack tree remains inactive until publish validation passes |
| TOML span not preserving comments or unknown fields | automatic fix refuses with manual steps |
| runtime-state conflict before migration marker | report manual conflict; no merge or delete |
| final typed participation validation failure | abort publish and name failed validation hook |
| healthy city repeated fix | byte-identical inputs and stable phase-two report |

#### Role-Surface Ownership And Host-Core Patch Model
<!-- REVIEW: added per attempt14-role-surface-ownership -->

The chosen model is unchanged but now becomes a table-driven release gate:
host Core is auto-included by Gas City; public Gastown does not import Core;
public Gastown may patch only symbolic host-Core bindings through normal config
layering. Core owns a compatibility default maintenance-worker binding whose
default key is `dog`. That key is pack configuration, not SDK role logic.

`plans/core-gastown-pack-migration/role-surface.generated.yaml` is the binding
source of truth. Every row has source span, current role-bearing behavior,
final owner, allowed field or replacement mechanism, expiry slice, and proof
test. The initial disposition table must include:

| Surface | Final owner | Allowed mechanism | Expiry/test |
| --- | --- | --- | --- |
| Go identifiers and string literals for Mayor, Deacon, Witness, Refinery, Polecat, Boot, Crew, or Gastown | public Gastown, docs-only, or historical fixture | none in production Core/SDK logic | scanner negative fixtures before source deletion |
| API/dashboard `crew` vocabulary | neutral API/session metadata | config-supplied grouping, not hardcoded classification | OpenAPI/dashboard generated-type check if touched |
| Core maintenance `dog` default | Core pack config compatibility data | `[gc.bindings.maintenance_worker]`, `target_binding`, and symbolic patches | renamed-worker and omitted-worker tests |
| provider pack `dog` routes | provider pack with binding rewrite or explicit exception | resolve `core.maintenance_worker` or tabled compatibility exception | provider old/new witness tests |
| Core formula pools and `gc.routed_to` metadata | Core only when generic | binding reference, never role literal fallback | formula composition and emitted metadata tests |
| mail/nudge recipients | formula, order, pack, city, or session metadata | missing optional recipient is no-op; missing required recipient fails preflight | mail/nudge recipient tests |
| prompt fragments | owning pack | no implicit cross-pack lookup; duplicate or move fragments explicitly | rendered prompt goldens |
| public Gastown assets | public Gastown | Gastown roles, prompts, formulas, orders, scripts, docs, and examples | packcompat from exact public pin |
| historical docs and migration fixtures | docs-only or fixture | allowlist row with owner and expiry | docs lint classification |

`[[patches.agent]] name = "dog"` is compatibility-only and must not appear in
the activation public pack's active assets. Active public Gastown uses
`target_binding = "core.maintenance_worker"`. A patch targeting a missing host
Core, omitted binding, or missing effective agent fails before behavior
executes with the host-Core diagnostic named in the role-surface row; it does
not create a replacement Core agent.

#### Retired-Pack Rejection In Normal Resolution
<!-- REVIEW: added per attempt14-retired-pack-rejection -->

Retired Maintenance and generated local Gastown bytes may remain on disk for
rollback and operator evidence, but active production resolution must reject
them before behavior discovery. This is a loader invariant, not a doctor-only
cleanup rule.

`internal/packsource` is the sole classifier for retired sources. The
classifier result is required before config load, pre-resolution doctor, import
check/install, lock validation, cache lookup, public-source normalization,
materialization, prompt/template discovery, hook/formula/order/script discovery,
docs lint, generated-reference lint, and error rendering.

| Retired surface | Normal runtime behavior | Doctor diagnostic | Fix behavior |
| --- | --- | --- | --- |
| `.gc/system/packs/maintenance` preserved directory | ignored and never globbed | legacy preserved generated directory or custom fork | never delete; optional report only |
| `.gc/system/packs/gastown` preserved directory | ignored and never selected for public pin | legacy preserved generated directory or custom fork | never delete; optional report only |
| explicit generated Maintenance import | fail before behavior discovery after pre-resolution report | retired import, generated provenance | remove only through coordinator |
| explicit generated local Gastown import | fail before behavior discovery after pre-resolution report | retired import, generated provenance | rewrite to public pin only through coordinator |
| custom local Maintenance/Gastown fork | fail as unsupported active retired source | manual custom fork | no automatic fix |
| transitive `../maintenance` import | fail as unsupported transitive retired import | actionable pre-resolution report | remove/rewrite only generated/example source |
| stale synthetic public Gastown cache | ignored for public pin | stale synthetic cache | optional one-time promotion to ordinary cache key after exact digest validation |
| prompt/template/order/formula files under retired dirs | not discovered | stale active behavior source if imported | no direct behavior load |
| retired lock entries | invalid for new public pin | retired lock entry | preserving rewrite only through coordinator |

Tests must cover absent retired dirs, stale generated dirs with custom files,
explicit legacy imports, transitive imports, custom forks, stale synthetic
caches, stale prompt fragments, stale hook overlays, stale order/formula
definitions, and import/check/install paths. A loader that succeeds by reading
retired generated bytes is a failing test case.

#### Public Pin Phase Ledger And Version-Skew Matrix
<!-- REVIEW: added per attempt14-public-pin-ledger -->

`public-gastown-pins.yaml` has three phase records, not only two. The
`current_baseline` row records the currently shipped public pin
`sha:d3617d1319a1206ac85f69ba024ec395c49c6f4b` and its observed behavior.
The `compatibility` row records the first public pin consumed while bundled
Maintenance remains active. The `activation` row records the public pin
consumed by the candidate Gas City tree that removes Maintenance from normal
production loading.

Each phase row contains phase, immutable commit, durable public ref, source,
subpath, pack digest, active asset digest, behavior-manifest digest,
role-surface digest, ordinary remote-cache key, registry/direct-source equality
proof, old-binary content digest evidence, new-binary content digest evidence,
offline/cache result, duplicate-active matrix result, rollback or one-way
boundary, release-note requirement, transcript paths, and validator version.

The version-skew behavior is explicit:

| Scenario | Required result |
| --- | --- |
| `v1.2.1` plus current baseline pin | existing behavior unchanged and observed content digest recorded |
| `v1.2.1` plus compatibility pin | loads public pack without relying on new loader features; duplicate-active matrix is green |
| `v1.2.1` plus activation pin | either supported with observed content digest evidence or named as a one-way boundary before activation |
| new binary plus current baseline pin | pre-resolution doctor reports version skew without mutating custom content |
| new binary plus compatibility pin | current-loader packcompat green; no synthetic alias selected |
| new binary plus activation pin | no-Maintenance production-loader packcompat green on the same candidate tree |
| rollback after doctor fix | old binary can read mutated manifests, or release notes name exact manual recovery |
| ordinary remote cache hit | exact source, commit, subpath, and digest satisfy the pin without network |
| offline no-cache | explicit failure; no embedded public Gastown or retired synthetic fallback |
| stale synthetic cache present | ignored unless explicitly promoted into the ordinary cache key after digest validation |

The public pin validator must observe file content digests from the actual
checkout or cache path used for pack installation. Load success alone is not
evidence.

#### Binding Slice Gates And Activation Commit Ordering
<!-- REVIEW: added per attempt14-slice-gate-artifact -->

`plans/core-gastown-pack-migration/slice-gates.generated.yaml` is the only
binding source of truth for rollout gates. Each row names slice id, owning repo,
exact commands, package targets, `-run` filters, old/new binary fixtures,
cache/offline fixtures, generated artifacts, docs artifacts, stale-path scans,
packcompat mode, expected public pin phase, and post-slice forbidden states.

Every high-risk slice runs `make test-fast-parallel` and `go vet ./...`.
Process and integration coverage is required for Core loading/doctor,
Maintenance folding, and registry/cache cleanup through the sharded targets in
`TESTING.md`; those commands appear in the gate artifact rather than only in
prose.

Activation is split into green intermediate commits:

| Commit step | Allowed change | Required green gate before next step |
| --- | --- | --- |
| A | land Core-owned Maintenance behavior behind existing active owners | behavior manifest rows and Core formula/script witnesses |
| B | land activation public Gastown pin in the candidate branch | public pin ledger validator and current duplicate matrix |
| C | switch `PublicGastownPackVersion` to activation pin | packcompat activation fixture setup and old/new binary matrix |
| D | remove Maintenance from `requiredBuiltinPackNames` on the same candidate tree | no-Maintenance production-loader packcompat through `internal/systempacks` |
| E | remove retired registry/cache aliases and stale discovery paths | stale-cache rejection, retired-source classifier, and process/integration shards |
| F | delete in-tree source only after replacement witnesses are green | old-test to new-test map, docs lint, stale-path scan |

No commit may expose duplicate active definitions, zero active owner for a
preserved behavior row, or a no-Maintenance claim backed by copied fixtures,
test-only loaders, or hidden bootstrap paths.

#### Behavior Evidence First Slice And Pilot Rows
<!-- REVIEW: added per attempt14-behavior-evidence-pilot -->

The first implementation slice produces evidence before any destructive source
move, role-neutral rewrite, public pin update, registry/cache retirement, docs
claim, or Maintenance removal. It generates and validates:

- `behavior-manifest.generated.yaml`
- independent old-tree baseline transcript
- `role-surface.generated.yaml`
- `loader-inventory.generated.yaml`
- `system-pack-wording.generated.yaml`
- `test-migration.generated.yaml`
- `public-gastown-pins.yaml`

The first slice must include pilot rows for the most fragile behavior before it
can claim the generator is adequate:

| Pilot behavior | Minimum old witness | Minimum final witness |
| --- | --- | --- |
| shutdown-dance requester/detector semantics | old formula/prompt render plus route/request transcript | Core generic render plus public Gastown detector/requester preservation |
| spawn-storm escalation | old order/script execution or golden side effect | Core script execution with same threshold/notification semantics or approved delta |
| `DOG_DONE` nudges | old nudge/mail route test | binding-resolved Core/public route test |
| JSONL/reaper notifications | old script side effect and notification target | Core runtime-state copy plus configured-recipient behavior |
| Polecat handoffs | old formula composition and routed metadata | public Gastown formula composition and packcompat route proof |
| branch pruning | old order/script execution | public Gastown order/script execution |
| review checks | old check/formula witness | final owner witness and moved-test mapping |
| doctor scripts | old diagnostic/fix golden | read-only plain doctor golden and coordinator fix golden |
| prompt fragments | old rendered prompt | owner-pack rendered prompt with no unresolved fragments |
| `session_live` hooks | old hook installation/execution witness | final owner hook installation/execution witness |
| public Gastown archive authorship | old author/committer behavior | public pack witness or approved semantic delta |

Execution-level old witnesses require execution-level final witnesses. Source
existence, path counts, or documentation rows cannot replace a command,
formula, hook, prompt render, runtime-state mutation, or script witness.

#### Bootstrap Fixture Isolation And Hidden Dependency Table
<!-- REVIEW: added per attempt14-bootstrap-fixture-seam -->

Bootstrap has one concrete seam: production bootstrap exposes a private
non-nil empty filesystem; tests inject inline `_test.go` `fstest.MapFS`
fixtures through a test-only helper. No production code or non-bootstrap test
may read production Core through `internal/bootstrap`, copied `testdata`, or
`GC_BOOTSTRAP=skip`.

| Current dependency class | Required migration |
| --- | --- |
| `internal/bootstrap` unit tests | inline `fstest.MapFS` with synthetic paths such as `packs/test-core` |
| non-bootstrap tests needing Core content | use `internal/packs/core.PackFS` or `internal/systempacks` helpers |
| config package tests needing no-Core behavior | call lower-level `internal/config` loaders directly as config tests |
| command/runtime tests | use production `internal/systempacks` loading; no environment escape hatch |
| `GC_BOOTSTRAP=skip` branch | delete production behavior or narrow to empty fixture materialization with tests proving Core still materializes |
| path-string guards | fail on `internal/bootstrap/packs/core`, `AssetDir: "packs/core"`, old hook overlay paths, and copied production Core fixtures outside allowlist |
| generated docs and packlint fixtures | update to `internal/packs/core` or synthetic fixture paths |

`TestBootstrapFixtureIsMinimal` fails if bootstrap fixtures include
`formulas/`, `orders/`, `overlay/`, `skills/`, `assets/prompts/`, scripts, or
copied production Core files. `TestProductionBootstrapAssetsIsEmpty` proves the
production filesystem returns `fs.ErrNotExist` and is never nil.

#### Docs, Wording, And Operator-Lint Schema
<!-- REVIEW: added per attempt14-docs-wording-linter -->

The docs and wording gate is generated and bidirectional. The canonical
operator reference is `docs/reference/system-packs.md`, nav-registered before
the first operator-facing behavior change. Public Gastown has a companion docs
artifact that consumes the same wording matrix.

`plans/core-gastown-pack-migration/system-pack-wording.generated.yaml` has:

- schema version, generator version, owners, and source artifact digests
- allowed contexts and forbidden contexts
- exact recommended phrases and accepted variants
- replacement phrases for retired wording
- consumers: docs, MDX, JSON, TXT, TOML, Go strings, shell, TypeScript,
  generated schemas, generated references, CLI help, doctor JSON/text, examples,
  scripts, pack comments, public-pack docs, troubleshooting, and tutorial
  transcripts
- freshness test ids, golden-test ids, and release-gate ids

The linter classifies every retained `maintenance`, `packs/maintenance`,
`.gc/system/packs/gastown`, `.gc/system/packs/maintenance`,
`.gc/runtime/packs/maintenance`, `dog`, `crew`, and `gastown` hit as one of:
retired-pack diagnostic, stale legacy state, runtime-state migration, valid
store/Dolt maintenance terminology, Core maintenance-worker terminology,
active public Gastown reference, public Gastown docs, generated fixture,
historical archive, or error.

The system-pack reference page must cover Core required-host behavior,
provider host packs, explicit public Gastown imports, retired Maintenance,
host-Core patch semantics, public pin version skew, offline/cache behavior,
runtime-state migration, doctor read-only versus `--fix`, and rollback or
one-way-boundary guidance.

#### Provider-Pack Continuity And Rewrite Exceptions
<!-- REVIEW: added per attempt14-provider-continuity -->

Provider-pack byte continuity and role cleaning are reconciled through a single
exception ledger. Untouched provider assets must remain byte-identical across
Core repair, provider materialization, stale retired-directory containment, and
offline self-heal. Any active provider role-bearing value that changes must
have a manifest row, an exception row, old/new witnesses, provenance, and an
expiry or compatibility rationale.

| Pack | Default rule | Allowed exception | Required proof |
| --- | --- | --- | --- |
| `bd` | byte-identical provider assets | active role-bearing command, doctor, script, template, formula target, or operator output discovered by role-surface inventory | old/new witness, semantic-delta record, untouched-file digest proof, provider matrix |
| `dolt` | byte-identical provider assets | rewrite `mol-dog-*`, mail/nudge recipients, formula pools, and related target metadata to `core.maintenance_worker` bindings when active behavior depends on the Core worker | backup, doctor, stale-db, phantom-db, compactor, health JSON, order pool, empty-recipient, and configured-recipient tests |
| provider locks and installed metadata | preserve lexical and semantic content | only update fields required by exact source/version/cache identity | lockfile preservation golden and downgrade/rollback diagnostic |
| generated comments or historical issue text | docs-only by default | rewrite only if surfaced in operator output or generated references | docs/wording linter row and golden |

Core repair tests must prove provider assets are isolated from Core
prune/quarantine repair. Offline old-cache-to-new-binary tests must prove `bd`
and `dolt` self-heal without network and without selecting retired Maintenance
or public Gastown synthetic aliases.

#### Attempt 14 Unfixable Items
<!-- REVIEW: added per attempt14-unfixable-items -->

None. The design remains `iterate` because the global verdict is `block`; every
blocker and major finding above is converted into an implementation contract,
artifact, table, or gate rather than accepted as residual risk.

### Attempt 17 Review Resolution Contracts
<!-- REVIEW: added per attempt17-global-synthesis -->

Attempt 17 keeps the review verdict at `iterate`. These contracts supersede
weaker earlier language that allowed prose-only gates, path/count witnesses,
loader allowlists without generated ownership, dog/Core ambiguity, or deferred
docs/runtime decisions. No blocker or major finding is accepted as residual
risk.

#### Default-Deny Production Loader Inventory
<!-- REVIEW: added per attempt17-required-core-loading -->

`plans/core-gastown-pack-migration/loader-inventory.generated.yaml` is the
binding source for all production config reads. The generator scans non-test
Go under `cmd/gc` and production `internal/` packages, follows aliases and
wrappers, and emits a row for every call that can reach `config.Load`,
`config.LoadCity`, `config.LoadWithIncludes`, `LoadWithIncludesOptions`,
no-refresh variants, raw TOML pre-reads, provider peeking helpers, completion
and stop helpers, controller/API reload helpers, command diagnostics, packman,
cache/lock validation, and any exported helper that returns a `*config.City` or
equivalent resolved config.

Each row contains file, function, symbol reached, loader class, fields read,
normal-runtime reachability, required-system-pack participation expectation,
allowed partial-read reason, owner, expiry, and focused test. The default is
deny: a production loader row is illegal unless it routes through
`internal/systempacks.LoadRuntimeCity`,
`internal/systempacks.LoadRuntimeCityNoRefresh`, or one of the generated
partial-read helpers named in the row.

`RequiredSystemPackParticipation` is resolver-produced, not inferred from paths.
It is matched to the validated file-set digest for Core, `bd`, and `dolt`, and
records the provider-pack selection algorithm used for that city. Provider-pack
selection is deterministic: Core is always required; `bd` and `dolt` are added
only from the city's configured beads/provider requirements through
`internal/systempacks.RequiredPackPlan`; public Gastown, retired Maintenance,
stale materialized directories, and synthetic aliases never influence the host
provider-pack set.

Failure behavior is loader-class specific:

| Loader class | Broken required pack result | May repair? | May read behavior? |
| --- | --- | --- | --- |
| runtime | fatal diagnostic before config behavior is read | yes, through system-pack repair before resolution | no until typed participation is valid |
| no-refresh runtime | fatal diagnostic if existing materialization is missing, stale, partial, corrupt, shadowed, or absent from participation | no refresh; may only report | no |
| partial edit | scoped raw TOML parse only | no | no agents, formulas, orders, prompts, hooks, or overlays |
| partial doctor report | scoped raw TOML plus retired-source classification only | no | no |
| doctor `--fix` | staged repair through mutation coordinator | yes, only under coordinator | only final validation after staged repair |
| test fixture | lower-level config tests only | test-owned | test contract names the bypass |

The broken-state matrix includes missing, corrupt, partial, stale,
unexpected-effective-file, materialized-but-unresolved, shadowed, same-name
collision, read-only city, repairable generated file drift, no-refresh stale
content, stale synthetic cache, and unsupported transitive retired import.
Plain diagnostics use read-only or no-refresh APIs. Mutation is limited to
explicit fix paths under the mutation coordinator.

#### Public Gastown, Core Dog, And Retired Maintenance Boundary
<!-- REVIEW: added per attempt17-public-gastown-core-boundary -->

Chosen model: public Gastown is a public pack that depends on host Core by
contract, not by import. Public Gastown never imports Core or Maintenance. It
may patch the host Core maintenance worker only through normal config layering
after required host packs participate in resolution.

Ownership is fixed:

| Surface | Final owner | Binding rule |
| --- | --- | --- |
| Core maintenance-worker base agent, compatibility name `dog` | Core | Default pack configuration only; Go must work if renamed or omitted. |
| Generic maintenance prompt body and Core prompt fragments | Core | Role-neutral text only; no Gastown requester/detector examples. |
| Gastown-specific dog prompt wording, workdir/theme patch, requester/detector behavior, and notifications | public Gastown | Patch host Core fields or own Gastown prompts; every patch has a behavior-manifest row. |
| Duplicate prompt fragments across Core and public Gastown | no implicit sharing | Names are pack-scoped; duplicate active global names are fatal unless a manifest row declares an alias. |
| `../maintenance`, `.gc/system/packs/maintenance`, transitive Maintenance imports | retired | Pre-resolution doctor may diagnose generated cases; production resolution rejects or ignores as classified stale state. |
| provider maintenance behavior | provider pack or Core | Provider-specific bytes stay provider-owned; active Core-worker bindings use config values. |

Rendered prompt tests compile Core-only, public-Gastown-with-host-Core, renamed
maintenance-worker, omitted-maintenance-worker, stale-Maintenance-import, and
duplicate-fragment fixtures. A public Gastown asset ledger is generated from
the exact pinned checkout and records active assets, patch targets, rendered
prompt digests, formula/order/script paths, and test ids. Source deletion waits
until each old Gas City asset maps to Core, public Gastown, provider pack,
compatibility, docs-only, or retired with approved evidence.

#### Ownership-Aware Role-Neutrality Scanner
<!-- REVIEW: added per attempt17-role-neutrality -->

Role neutrality is enforced by `role-surface.generated.yaml` plus a scanner
that understands identifiers and ownership. Matching splits snake_case,
kebab-case, dot paths, camelCase, PascalCase, TOML keys, metadata keys,
function names, generated OpenAPI/dashboard strings, comments, templates,
scripts, and prompt prose. It rejects sub-identifier surfaces such as
`DogTheme`, role icon maps, theme functions, warmup defaults, prompt fallbacks,
`crew` API/dashboard vocabulary, `gastown` template wiring, public-pack
constants used by Core, and dispatch formula-name heuristics unless a row
allows them.

Each row contains file/asset, span, matched token, ownership class, final
disposition, replacement mechanism, slice id, proof test, owner, and expiry.
Core-owned rows fail on hardcoded Gastown roles. Public Gastown rows preserve
Gastown behavior as inventory and fail only when they import retired
Maintenance, require host internals, or leak into Core. Provider-pack rows are
byte-preservation by default and require exception-ledger proof for active
rewrites.

The Go migration table must include at least these dispositions:

| Surface | Required disposition |
| --- | --- |
| theme/icon helpers and `DogTheme`-style APIs | replace Go role defaults with config-driven display metadata or public Gastown assets |
| API/OpenAPI/dashboard `crew` vocabulary | replace with neutral session grouping or config-supplied labels; regenerate schema/types in the same slice |
| warmup mail/default city scaffolding | Core role-neutral defaults; Gastown defaults live in public Gastown templates |
| prompt fallback/help text | Core neutral fallback; Gastown examples in public docs/assets |
| sling/formula heuristics | formula selection from config/metadata, not role-name strings |
| provider `dog`, `mayor`, or `deacon` bindings | resolve configured Core maintenance worker or configured recipients |
| removed role-name guards | replace with generated scanner rows and tests, not ad hoc Go conditionals |

If a Go role surface is intentionally deferred, the design claim for that slice
must narrow to exclude it and the row must name the follow-up bead. Otherwise
the slice is blocked.

#### Pin, Cache, Activation, And Rollback Ledger
<!-- REVIEW: added per attempt17-rollout-cache-rollback -->

`public-gastown-pins.yaml` remains three-row and now has mandatory old/new
binary evidence for each row: `current_baseline`, `compatibility`, and
`activation`. Each row records immutable commit, durable public ref, subpath,
pack digest, active-asset digest, behavior-manifest digest, role-surface
digest, ordinary remote-cache key, registry/direct-source equality transcript,
old-binary result, new-binary result, duplicate-active result, offline/cache
result, rollback class, release-note requirement, and validator version.

Activation pin switch and Maintenance removal are one candidate gate unless
the ledger proves current-loader coexistence. The activation candidate must run
normal production loading with Maintenance absent, old-binary plus activation
pin or a named one-way boundary, stale synthetic-cache rejection, ordinary
remote-cache hit, offline no-cache failure, downgrade after doctor fix, and
zero-duplicate/no-gap checks.

Legacy synthetic cache handling is ordered before ordinary git validation:
retired synthetic public Gastown and Maintenance entries are detected,
classified, and ignored. Promotion into an ordinary remote-cache key is allowed
only through an explicit digest-validated helper that records source, commit,
subpath, old synthetic key, new ordinary key, and transcript. Downgrade and
rollback artifacts are generated beside the pin ledger and name which manifests
old binaries can read, which state remains legacy-readable, and which manual
steps are required for a one-way activation.

#### Doctor Mutation And Runtime-State Safety
<!-- REVIEW: added per attempt17-doctor-runtime-safety -->

Plain `gc doctor` is report-only. It may run pre-resolution TOML import parsing,
retired-source classification, lock/cache inspection, required-pack integrity
checks, and runtime-state conflict detection, but it must not materialize,
repair, promote cache entries, write locks, quarantine/prune files, migrate
runtime state, or rewrite TOML.

All mutation belongs to `gc doctor --fix` through a mutation coordinator. The
coordinator owns one operation plan with a generation id, advisory city lock,
post-lock digest refresh, staged Core/provider/public-pack content, scoped
TOML/CST edits, staged runtime-state copies, compare-before-rename writes,
final validation, and a publish record. A failed preflight leaves every live
file byte-identical. A failed publish records the exact generation and file
state so a later `--fix` can resume or report manual recovery without guessing.

Runtime-state migration has a binding table:

| State | Legacy read path | Core write path | Doctor/fix behavior | Rollback behavior |
| --- | --- | --- | --- | --- |
| JSONL export state and `.bak` | `.gc/runtime/packs/maintenance/jsonl-export-state.json*` | `.gc/runtime/packs/core/jsonl-export-state.json*` | copy only when Core destination absent; conflict is manual before marker | legacy retained for old binaries |
| JSONL archive repo | `.gc/runtime/packs/maintenance/jsonl-archive/` | `.gc/runtime/packs/core/jsonl-archive/` | copy repo/refs/remotes only when destination absent | legacy retained; no merge after marker |
| push-failure counters and export cursors | keys inside JSONL state | same keys in Core state | preserve values exactly | legacy values remain readable |
| spawn-storm ledger | `.gc/runtime/packs/maintenance/spawn-storm-counts.json` | `.gc/runtime/packs/core/spawn-storm-counts.json` | copy when destination absent; Core wins after marker | legacy ignored but retained |
| order skips and aliases | bead/order metadata | existing names or alias rows | preserve names or add alias migration test | old names remain diagnosable |
| provider health JSON consumers | provider/Core state paths | provider/Core owner row | owner-specific witness required | no path rewrite without exception row |

Doctor concurrency uses live process/runtime discovery, not PID, lock, or status
files as authority. Healthy `--fix` is a zero-write golden. Generated-vs-custom
pack detection uses digest, generation marker, source path, lock metadata, and
operator edit classification; custom forks are manual diagnostics.

#### Generated Artifact Contracts And Independent Completeness
<!-- REVIEW: added per attempt17-generated-artifacts -->

The following artifacts are generated, checked in, and freshness-tested before
implementation beads may move or delete behavior:

| Artifact | Owner | Required command/test | Required schema content |
| --- | --- | --- | --- |
| `behavior-manifest.generated.yaml` | Gas City plus public Gastown | generator command, old-tree transcript, `test/packcompat` | row id, old/final owner/path/digest, helper deps, trigger, observable output, semantic delta, old/final witnesses, public commit |
| `role-surface.generated.yaml` | Gas City | scanner test | token span, ownership class, disposition, replacement, allowlist expiry, proof test |
| `loader-inventory.generated.yaml` | Gas City | loader scanner test | call site, reached symbol, loader class, wrapper/allowlist, fatality boundary, proof test |
| `slice-gates.generated.yaml` | Gas City | gate validator | slice id, exact commands, packages, `-run` filters, artifacts, forbidden states, public pin phase |
| `public-gastown-pins.yaml` | Gas City plus public Gastown | pin validator | three phase rows, old/new binary transcripts, cache/offline/rollback evidence |
| `system-pack-wording.generated.yaml` | Gas City plus public Gastown | docs/wording linter | term, allowed contexts, forbidden contexts, replacement phrase, consumers, goldens |
| `test-migration.generated.yaml` | Gas City plus public Gastown | test-map validator | old test/subtest, replacement test/subtest, removal approval, behavior row id |
| `provider-exceptions.generated.yaml` | Gas City | provider continuity tests | provider asset, default byte policy, allowed rewrite, old/new witness, expiry |

Completeness is independently checked from VCS history and transcripts. The
behavior generator cannot self-certify solely from the current tree: it must
compare old-tree file moves/deletions/additions, old execution transcripts,
current generated rows, public pinned-checkout rows, and test-migration rows.
Any source move, deletion, added replacement asset, removed test assertion, or
changed operator output without a row fails CI. Implementation beads cite row
ids from these artifacts; prose sections are advisory when they conflict with a
generated row.

#### Bootstrap Embed Removal And Hidden Dependency Guard
<!-- REVIEW: added per attempt17-bootstrap-hidden-dependencies -->

Bootstrap cleanup is source-symbol guarded, not path-string-only. The hidden
dependency guard scans imports, symbols, tests, docs, generated references, and
fixtures for `bootstrap.BootstrapPacks`, `bootstrap.PackNames`,
`bootstrapSkillDirs()`, `GC_BOOTSTRAP`, old bootstrap Core imports, skill
materialization, cache helpers, provider registry functions, hook overlays, and
prompt tests.

The Core extraction slice deletes the production `//go:embed packs/**`
directive and replaces `bootstrapAssets` with a private non-nil empty
filesystem that returns `fs.ErrNotExist`. Production bootstrap assets must be
empty and never read. Tests that need bootstrap fixtures inject inline
`fstest.MapFS`; command/runtime tests use `internal/systempacks`; config
package tests that need no-Core behavior use lower-level config helpers only.

`GC_BOOTSTRAP=skip` is deleted from production semantics unless it can be
proved to skip only empty bootstrap fixture materialization. It cannot skip
Core materialization, provider host packs, retired-source classification,
cache/lock validation, collision checks, or typed participation. Core skill
resolution moves to Core/systempack sources and has a materialization test that
fails if any production path reads `internal/bootstrap/packs/core`.

#### Operator Docs, Terminology, And Release Evidence
<!-- REVIEW: added per attempt17-operator-docs -->

Docs are bound to runtime-state and terminology decisions before release. The
canonical system-pack reference is `docs/reference/system-packs.md`, registered
in docs navigation. Public Gastown owns a companion reference that uses the
same wording matrix.

The wording linter covers Markdown, MDX, JSON, TXT, TOML, Go strings,
TypeScript, generated schema/reference outputs, CLI/help, doctor text and JSON,
`FixHint` strings, tutorials, troubleshooting, examples, scripts, pack comments,
public Gastown docs, and docs navigation. It distinguishes retired standalone
Maintenance, lower-case maintenance, store/Dolt maintenance, Core maintenance
worker, compatibility name `dog`, dog pool examples, public Gastown, host Core,
legacy `.gc/system/packs/maintenance`, legacy `.gc/runtime/packs/maintenance`,
and Dolt maintenance settings. Same-slice docs/schema/help updates are release
gates for the behavior they describe.

Operator transcript coverage is required for fresh init, upgraded local
Gastown imports, retired Maintenance imports, stale system-pack directories,
runtime-state migration, `gc doctor` report-only output, `gc doctor --fix`
success, `--fix` refusal on custom forks, version skew, offline no-cache, stale
synthetic cache, and rollback or one-way activation recovery.

#### Witness Rows For Behavior And Provider Continuity
<!-- REVIEW: added per attempt17-behavior-provider-witnesses -->

Behavior preservation requires exact row witnesses before destructive changes.
The first generator pass must include rows for branch pruning, Polecat formulas,
shutdown-dance detectors/requesters, review workflows, commands, doctor checks,
overlays, prompts, role agents, provider pack scripts, warrants,
notifications, and public Gastown authorship. Initial public Gastown path rows
must name final paths such as `gastown/orders/prune-branches.toml`,
`gastown/scripts/prune-branches.sh`,
`gastown/formulas/mol-polecat-base.toml`,
`gastown/formulas/mol-polecat-commit.toml`,
`gastown/formulas/mol-polecat-report.toml`, and any prompt or fragment path
that preserves removed Core/Maintenance text. If a final public path is not
known, the row is `blocked:path-unresolved` and the source cannot be moved.

Provider continuity has exception rows for more than byte identity:

| Surface | Required witness |
| --- | --- |
| `dolt-target.sh` and cross-pack sourced scripts | old/new script execution, pack-relative path, env, and author/provenance output |
| configured recipients and empty recipient handling | old/new mail or nudge route tests with configured and empty values |
| `pool = "dog"` or hardcoded worker targets | configured Core maintenance-worker resolution and renamed-worker fixture |
| `DOG_DONE` nudges | old route witness and final binding-resolved route witness |
| author/committer identities | old/new Git/Dolt author transcript or approved semantic delta |
| health JSON consumers | old/new consumer path, schema, and provider/Core owner witness |
| provider host-pack selection | `bd`, `dolt`, and exec provider matrices through `RequiredPackPlan` |
| Core repair isolation | untouched provider file digest proof after Core repair and offline self-heal |

Byte identity remains the default for provider assets. Any active provider
rewrite needs a manifest row, provider exception row, old/new witness, semantic
delta, and expiry or compatibility rationale.

#### Attempt 17 Unfixable Items
<!-- REVIEW: added per attempt17-unfixable-items -->

None. The remaining state is `iterate` because the global verdict is `block`
and the next review must verify these new contracts against the rest of the
design.
...
- The replacement commit must be immutable and recorded in the manifest. Gas
  ...
  compatibility commit for coexistence work, and must name the activation
  commit in the same candidate branch that removes Maintenance or deletes
  source.
...
mode. At activation-pin adoption time, on the same candidate tree that removes
Maintenance from required system packs, the same gate must run in no-Maintenance
...
- Fresh `gc init --template gastown` resolves the public pack from the network
  source or from an ordinary remote-pack cache that already contains the exact
  public pin. If neither source is available, the error must say fresh Gastown
...
- Required-pack materialization repairs missing/corrupt expected files and
  prunes or quarantines unexpected effective files before validation; behavior
  discovery never sees a partially repaired required pack.
...
- Pack/Core/import and runtime-state checks submit fix intents to the
  ...
  migration.
- The coordinator acquires the city advisory lock before mutating preflight. If
  any report-only read happened before the lock, the coordinator repeats all
  file digests, lockfile reads, source provenance, and runtime-state conflict
  checks after the lock before staging.
- Preflight must run before any mutation. It verifies the public Gastown source
  and immutable version are reachable or present in a validated ordinary
  remote-cache entry, installable, and lockable; generated Core can satisfy
  strict file-set integrity after prune/quarantine repair; existing lockfiles are
  parseable; city manifests can be edited without dropping unknown syntax; and
  runtime-state conflicts are classified.
...
- Runtime state under `.gc/runtime/packs/maintenance` follows the concrete
  ...
  ledgers copy to `.gc/runtime/packs/core` when safe and write the
  migration-completed marker; conflicts are manual only before that marker
  exists or when marker provenance does not match. Ignored legacy paths are
  never deleted by `gc doctor --fix`.
...
- Add guards that fail on lingering imports of
  `internal/bootstrap/packs/core`, `AssetDir: "packs/core"` in bootstrap
  fixtures, and file copies/hashes that still target the old production Core
  path.
- The hidden-dependency inventory for this slice explicitly includes
  `cmd/gc/prompt_test.go`, `internal/config/bundled_import_test.go`,
  `examples/gastown/precompact_hook_test.go`, `test/packlint/*`, generated docs,
  and hook/config references. `cmd/gc/prompt_test.go` must be updated to read
  from `core.PackFS` or the new `internal/packs/core` path rather than the old
  bootstrap path.
...
- Remove `examples/gastown/packs/maintenance` only in the activation candidate
  tree after Core/public Gastown replacement witnesses are present; merge waits
  for the activation-pin no-Maintenance gate on that same tree.
...
- Behavior proof for Core formulas and orders: compose representative formulas
  into molecules, assert resolved step content rather than counts alone, verify
  run-target bindings, pools, hook targets, configured agents, configured
  recipients, and execute moved scripts from pack-relative paths.
...
- Provider pack continuity: `bd` and `dolt` materialized provenance and
  untouched asset bytes remain unchanged except for expected manifest metadata;
  any role-cleaning or target-binding rewrite has a behavior manifest row,
  old/new witness tests, and provider matrices still resolve the correct
  required packs.
- Core integrity: tampered Core is repaired; unexpected effective files are
  pruned or quarantined before strict full file-set integrity validation and
  before behavior discovery.
...
2. Gas City public-pin adoption and packcompat slice:
   ...
   away from local `../maintenance` in this slice or earlier, and update
   `examples/dolt` formulas and references that depend on legacy Maintenance
   scripts or role targets. Before this slice exposes the new pin to existing
   city diagnostics, legacy whole-file automatic import rewrites are disabled or
   routed through the new mutation coordinator; unsafe legacy `gc doctor --fix`
   behavior must not run in slice 2 or 3 binaries.
   Required gates:
   ordinary remote-pack install for the exact pin,
   `go test ./test/packcompat -run TestPinnedPublicGastownBehavior` including
   hermetic local-fixture/cache mode,
...
5. Public Gastown activation and Maintenance folding slice:
   ...
   commit to the activation commit from slice 1 and remove Maintenance from
   `requiredBuiltinPackNames` in the same candidate branch. The no-Maintenance
   production-loader packcompat gate runs on that combined tree before merge.
   If the activation commit cannot support old/new binary and
   duplicate-definition requirements, stop and convert this to a paired
   cross-repo activation/removal boundary with explicit rollback instructions.
   Move Core-owned Maintenance assets into Core, consume the already-landed
   Gastown-owned activation assets from public Gastown, update
   `jsonl_archive_doctor_check.go` to prefer Core runtime state, and apply the
   runtime retirement table. Required gates: formula/order/script behavior
   tests, stale directory isolation and preservation tests, runtime-state
   migration completion/no-false-conflict diagnostics, provider pack continuity
   and role-cleaning provenance tests,
   `go test ./test/packcompat -run TestPinnedPublicGastownBehavior` in
   no-Maintenance production-loader mode, `make test-cmd-gc-process-parallel`,
   `make test-integration-shards-parallel`, `make test-fast-parallel`, and
   `go vet ./...`.
...
- Existing cities with legacy local Gastown imports get rewritten to the public
  Gastown pack by `gc doctor --fix` only through the mutation coordinator and
  only when generated/example provenance is proven.
- Existing cities with legacy Maintenance imports get those imports removed by
  `gc doctor --fix` only through the mutation coordinator and only for
  generated/example Maintenance sources; Core supplies generic maintenance and
  public Gastown supplies Gastown-specific behavior.
...
- The third risk is public pack version skew. Land `gascity-packs` first and
  ...
  through the network source or a validated ordinary remote-cache entry.
- The fourth risk is accidental deadlock in recovery tooling. The mitigations
  are lock-first doctor mutation, prune/quarantine repair before strict
  required-pack validation, and migration-completed markers that prevent retained
  legacy state from becoming repeated false conflicts.
...
`Cross-Pack Ownership Decisions`. Core/Gastown/provider ownership audits must be
resolved in the candidate public Gastown slice before either public pin is
consumed. Later move/delete slices may complete only local witness wiring named
by an already-resolved row; they may not defer ownership decisions, provider
role-cleaning exceptions, or replacement-test ownership.
<!-- REVIEW: added per attempt13-audit-timing -->
