# Dimitri Volkov — DeepSeek V4 Flash Perspective Independent Review (Iteration 13 / Attempt 13)

**Verdict:** approve-with-risks

**Scope:** Test-Slicing, Migration Gate Ordering, Coverage Continuity, and Fixture Drift Detection.
**Reviewed design:** Iteration 13 Design Document (`updated_at: 2026-06-07T14:05:04Z`, 2732 lines), comparing with requirements, codebase realities, and historical wave findings.

---

## Executive Summary

As Dimitri Volkov, the **Test-Slicing Coverage Verifier**, I have conducted an exhaustive, independent review of the Iteration 13 Design Document.

The Iteration 13 revision represents a massive leap forward in architectural rigor and rollout safety. By adopting the **Double-Pin Rollout Model (lines 1179–1215)**, incorporating a formal generated **System-Pack Wording Matrix**, and introducing the **Retired-Source Classifier (lines 1216–1235)**, the design successfully resolves the circular examples-tree import dependencies and rollout-collision deadlocks that blocked prior iterations. Decoupling the examples tree rewiring to Slice 2 or earlier (lines 2441–2447) and establishing an executable two-pin boundary with explicit rollback/one-way mappings are exemplary engineering practices.

However, from an uncompromising **Implementation Slicing and Gate Ordering** perspective, several critical logical contradictions, stale instructions, and coverage omissions survive in the Iteration 13 draft. Specifically:
...
3. **The Bootstrap Test Fixture Sync and Drift Risk (Q3):** While the design proposes a testdata core fixture under `internal/bootstrap` to keep bootstrap tests isolated and deterministic after production Core is removed, it lacks any CI gate or freshness check to detect when this fixture copy diverges from the shipping `internal/packs/core`. This directly risks low-level test failure or silent coverage regression as the real Core pack evolves.
4. **Incomplete Slice Gate Matrix (lines 1417–1424):** The matrix still only names broad, narrative test targets, completely failing to specify concrete sharded process/integration targets (e.g., `test-cmd-gc-process-parallel` or `test-integration-shards-parallel` from `TESTING.md`) required by its own completeness contract (lines 1398–1401).
5. **Audit-Timing Wording Conflict:** "Open Questions" (lines 2728–2731) still states that audits "must be resolved in the slice that moves or deletes the relevant source," contradicting Slice 1 (lines 2618–2621) which requires all cross-pack ownership audits resolved *before* either pin is consumed.
6. **Dolt Example Slicing and Gate Omission:** While Slice 2 names rewiring for `examples/gastown` explicitly, no rollout slice is assigned ownership of updating the `mol-dog-*` formulas in `examples/dolt` which actively depend on legacy Maintenance scripts.
7. **Slice 1 "Replacement Tests" Gate is Unsatisfiable for Core-Bound Rows:** Slice 1 requires that all Core/Gastown ownership rows "have final Core/Gastown ownership rows and replacement tests" before either pin is consumed. But Core-bound rows cannot have final Core-side witnesses in Slice 1 because `internal/packs/core` is not extracted until Slice 3 (lines 2637–2643).
...
### 3. The Bootstrap Test Fixture Sync and Drift Risk
- **The Risk:** Slices 3 & 4 remove production Core from BootstrapPacks, shifting `internal/bootstrap` testing to a test-only Core fixture to preserve test determinism. However, the design does not specify how this test fixture is kept in sync with the real, evolving `internal/packs/core`.
- **The Gap:** If the test fixture diverges from the shipping Core pack, low-level configuration, loading, or hook installation tests may continue to pass against a "stale" mock world while failing in production against the actual Core pack.
- **Required Recommendation:** Require a dedicated freshness unit test (e.g., `TestBootstrapCoreFixtureFresh`) in `internal/bootstrap` that compares the test fixture's structure, files, and manifests with the production `internal/packs/core` during baseline testing, failing loudly on any unapproved divergence.
...
### 5. Audit-Timing Wording Conflict
...
### 6. `examples/dolt` Slicing and Gate Omission
...
### 7. Slice 1 "Replacement Tests" Gate is Unsatisfiable for Core-Bound Rows
...
- **Verifier Finding: SATISFACTORY.** The combination of the "Behavior-Oriented Witness Floor" (lines 313–318) and the `packcompat` suite (line 1066) provides excellent behavior-level verification. No test can be retired without equivalent behavior-oriented execution proof on the other side. However, we must ensure "step count assertions" are replaced with "resolved step content: run-target binding, pool, and hook target" assertions.
...
- **Verifier Finding: AT RISK.** The design correctly introduces a decoupled test-fixture copy of Core to keep bootstrap unit testing hermetic and isolated. However, it lacks a mechanism or gate to prevent fixture drift as production Core evolves. Adding a freshness test (such as `TestBootstrapCoreFixtureFresh`) is required.
...
| **Fixture copy of Core diverges from shipped Core with no detection mechanism** | Stale bootstrap test fixtures mask real runtime breakages or hide regressions. | **GAP.** No gate or check is specified to ensure `internal/bootstrap` test fixture stays in sync with production `internal/packs/core`. | **At Risk** |
...
3. **Add Bootstrap Fixture Freshness Check:** Explicitly require a `TestBootstrapCoreFixtureFresh` unit test in `internal/bootstrap` to verify that any changes to `internal/packs/core` are actively mirrored or safely exempted in the test-fixture directory.
4. **Flesh Out Slice Gate Matrix:** Update lines 1417–1424 to name the exact process/integration shard targets (such as `test-cmd-gc-process-parallel` or `test-integration-shards-parallel`) for Slices 4, 5, and 6.
5. **Reconcile Audit-Timing Conflict:** Align lines 2728–2731 with Slice 1's gate, stating that ownership decisions are resolved in Slice 1, while witnesses land with the owning slice.
6. **Add Dolt Example to Slicing:** Assign the update of `examples/dolt` references and formulas to a concrete slice (e.g., Slice 2 or Slice 5).
7. **Reconcile Slice 1 Replacement Test Gate:** Restrict Slice 1's replacement tests gate to Gastown-side witnesses and Core row declarations, allowing Core witnesses to land in their respective extraction/folding slices.
