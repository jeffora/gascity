...
The behavior inventory is generated from source discovery, not hand-curated
...
| --- | --- |
| `.gc/runtime/packs/maintenance` | ignored legacy, migrated to Core, or manual diagnostic; never deleted by `doctor --fix`. |
| JSONL archive state | Core destination or ignored-legacy diagnostic with operator command guidance. |
| export cursors | Core destination or ignored-legacy diagnostic. |
| spawn-storm ledgers | Core destination or ignored-legacy diagnostic. |
...
2. Gas City public-pin adoption and packcompat slice:
   ...
   pinned-public-pack compatibility test without deleting in-tree sources.
   Required gates:
...
6. Registry/cache cleanup slice:
   ...
   packcompat test against the exact public pin.
