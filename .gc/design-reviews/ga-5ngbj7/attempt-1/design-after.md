...
| Related design | `DESIGN.md` owns architecture direction and extraction backlog. |
...
| SESSION-RECON-007 | Progress-aware health | When configured, an alive runtime with no progress after the threshold is not desired and can be drained/respawned. Attached sessions, startup grace, and pending interaction are exempt. Provider-health red takes precedence. | commit `dbda1e380`; `cmd/gc/session_progress_test.go`; `cmd/gc/session_reconciler.go` |
...
| SESSION-WORK-003 | Orphan pool step beads | Open pool step beads assigned to dead session identities are collected and released after session drain without relying on stale snapshots. | commit `8068393d8`; `cmd/gc/build_desired_state_test.go` |
