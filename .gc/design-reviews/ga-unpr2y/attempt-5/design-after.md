...
| Latest design-review disposition | Attempt 5 global verdict `block`; this revision is an `iterate` response |
...
Two items cannot be completed inside this design document alone: the
source-verified writer inventory and numeric performance baselines require
source artifacts and executable tests. This document therefore blocks
...
`cmd/gc/session_name_lookup.go`, `cmd/gc/session_circuit_breaker.go`,
`cmd/gc/soft_reload.go`, `cmd/gc/cmd_wait.go`, `cmd/gc/cmd_stop.go`,
`cmd/gc/cmd_nudge.go`, `cmd/gc/adoption_barrier.go`,
...
`internal/mail/beadmail/beadmail.go`, and `internal/extmsg`.
...
- `session_design.rollback=<files and switch to revert>`
