...
| Latest design-review disposition | Attempt 4 global verdict `block`; this revision is an `iterate` response |
...
`internal/mail/beadmail.go`, and `internal/extmsg`.
...
`durable-scan-blocked`, or `event-emission-failed` instead of claiming that the
system observed skipped event delivery.
