...
| SESSION-RECON-008 | Idle timeout ladder | An alive session idle past its threshold stops with sleep reason `idle-timeout` unless a timer blocker (user hold, quarantine) or pending interaction defers it. A pending interaction cancels any pending drain and keeps the session out of that tick's wake pass; blocker deferrals do not. Idle stops never consult assigned work. Precedence: blocker, then pending, then stop. | `internal/session/lifecycle_timers.go` (`DecideIdleTimeout`); `internal/session/lifecycle_timers_test.go`; `cmd/gc/session_reconciler_test.go` (`TestReconcileSessionBeads_IdleTimeout*`) |
